...
@Controller // servlet-context.xml에서 <context:component-scan base-package="org.zerock.controller" />용
@RequestMapping("/sample/") //http://192.168.111.104:80/controller/sample/모든것에 반응
@Log4j2 // resources폴더에 log4j2.xml필수
public class SampleController {
	...
	public String ex01(SampleDTO dto) { // SampleDTO dto = new SampleDTO();
		log.info("SampleController.basicGet2()메서드 실행");
		log.info("get방식에만 반응한다.");
		log.info("입력 dto확인 : " + dto);
		
		return "mbcex01"; // WEB-INF/views/ex01.jsp -> servliet-context.xml이 하는 일
	...
	public String ex02(@RequestParam("owner") String name,
		...
		// 프론트 변수와 백엔드의 변수가 다를때 (초보자는 일치 시킬것!)
		log.info("name : " + name);
		log.info("age : " + age);
	...
	public String ex04(SampleDTO dto, @ModelAttribute("page") int page) { // int page는 게시판에 페이징기법 예시
		//@ModelAttribute("page") 안쓰면 백앤드에는 전달 되지만 프론트에는 전달 안됨
	...
	public void ex05() {
		...
		// 리턴이 void인 경우 자동으로 [/WEB-INF/views/sample/ex05.jsp]를 찾는다.
		// 초보자 사용 금지
	}

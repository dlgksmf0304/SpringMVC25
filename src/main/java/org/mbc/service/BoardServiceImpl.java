...
@AllArgsConstructor // 생성자 자동주입 (new BoardServiceImpl(mapper);)
@Log4j2 // 로깅 처리용
public class BoardServiceImpl implements BoardService {
	// 조원이 실행문을 생성하는 용도로 활용 : implements BoardService (구현클래스)

	
	private BoardMapper mapper ; // DB쿼리용 객체(CRUD)
	
	@Override
	public void register(BoardVO board) {
		// 객체가 넘어오면 메퍼를 이용해서 데이터베이스에 등록하겠다.
	...
	public BoardVO get(Long bno) {
		...
		log.info("BoardServiceImpl.getList메서드 실행");
	...
	public boolean modify(BoardVO board) {
		// 프론트에서 객체가 넘어오면 메터를 통해서 update쿼리가 실행된다.
	...
	public boolean remove(Long bno) {
		// 프론트에서 번호가 넘어오면 메퍼를 통해서 delete 쿼리가 실행된다. 
